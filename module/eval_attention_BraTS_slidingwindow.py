...
def eval_net_dice(net, criterion, phase, network_switch, dataset, preview=True, gpu=False, visualize_batch=None, epoch=None, slice=127, root_path='no_root_path'):
        ...
        if preview:
            if visualize_batch is not None:
                if i == int(visualize_batch):
                    ...
                    fig = visualize_Seg(inputs_vis[0][0], labels_vis[0][0], outputs_vis[0][0], figsize=(6, 6), epoch=epoch, slice=slice)
...
def eval_net_mse(net, criterion, phase, network_switch, dataset, preview=True, gpu=False, visualize_batch=None, epoch=None, slice=127, root_path='no_root_path'):
    """Evaluation without the densecrf with the dice coefficient"""
    net.eval()
    running_loss = 0
    for i, b in enumerate(dataset):
        ...
        mask_predL, mask_pred,_ = net(img, phase, network_switch)
        # mask_pred = (mask_pred > 0.5).float()
