...
# class semiSupervised3D_attention(nn.Module):
#     def __init__(self, in_channels, n_classes, base_n_filter=8):
#         super(semiSupervised3D_attention, self).__init__()
...
#         self.upscale_conv_norm_lrelu_l0_seg = self.upscale_conv_norm_lrelu(self.base_n_filter * 16, self.base_n_filter * 8)
#         self.upscale_conv_norm_lrelu_l0_rec = self.upscale_conv_norm_lrelu(self.base_n_filter * 16, self.base_n_filter * 8)
#
#         self.conv3d_l0_seg = nn.Conv3d(self.base_n_filter * 8, self.base_n_filter * 8, kernel_size=1, stride=1, padding=0,
#                                    bias=True)
#         self.conv3d_l0_rec = nn.Conv3d(self.base_n_filter * 8, self.base_n_filter * 8, kernel_size=1, stride=1, padding=0,
#                                    bias=True)
...
#                                    bias=True)
#         self.conv3d_l1_ssl = nn.Conv3d(self.base_n_filter * 4, self.base_n_filter * 4, kernel_size=1, stride=1, padding=0,
#                                    bias=True)
...
#                                    bias=True)
#         self.conv3d_l2_ssl = nn.Conv3d(self.base_n_filter * 2, self.base_n_filter * 2, kernel_size=1, stride=1, padding=0,
#                                    bias=True)
...
#                                    bias=True)
#         self.conv3d_l4_ssl = nn.Conv3d(self.base_n_filter, 2, kernel_size=1, stride=1, padding=0,
#                                    bias=True)
#
...
#                 param.requires_grad = decoder_rec
#             for param in self.inorm3d_l0_rec.parameters():
#                 param.requires_grad = decoder_rec
...
#         # out = torch.cat([out, context_1], dim=1)  # 512
#         out_d2 = self.conv_norm_lrelu_l4_ssl(out_d2)  # 32
#         out_rec = self.conv3d_l4_ssl(out_d2)  # 2
#
#         return F.sigmoid(out_seg), F.sigmoid(out_rec)


class MASSL_norm(nn.Module):
    def __init__(self, in_channels, n_classes, base_n_filter=8):
        super(MASSL_norm, self).__init__()
        # Define basic parameters
        self.in_channels = in_channels
        self.n_classes = n_classes
        self.base_n_filter = base_n_filter
        ...
        self.e_conv3d_l2_1 = nn.Conv3d(self.base_n_filter, self.base_n_filter * 2, kernel_size=3, stride=2, padding=1,
                                   bias=True)
        self.e_conv3d_l2_2 = nn.Conv3d(self.base_n_filter * 2, self.base_n_filter * 2, kernel_size=3, stride=1, padding=1,
                                   bias=True)

        # Encoder Level 3 pathway
        self.e_conv3d_l3_1 = nn.Conv3d(self.base_n_filter * 2, self.base_n_filter * 4, kernel_size=3, stride=2, padding=1,
                                   bias=True)
        self.e_conv3d_l3_2 = nn.Conv3d(self.base_n_filter * 4, self.base_n_filter * 4, kernel_size=3, stride=1, padding=1,
                                   bias=True)

        # Encoder Level 4 pathway
        self.e_conv3d_l4_1 = nn.Conv3d(self.base_n_filter * 4, self.base_n_filter * 8, kernel_size=3, stride=2, padding=1,
                                   bias=True)
        self.e_conv3d_l4_2 = nn.Conv3d(self.base_n_filter * 8, self.base_n_filter * 8, kernel_size=3, stride=1, padding=1,
                                   bias=True)

        # Encoder Level 5 pathway
        self.e_conv3d_l5_1 = nn.Conv3d(self.base_n_filter * 8, self.base_n_filter * 16, kernel_size=3, stride=2, padding=1,
                                   bias=True)
        self.e_conv3d_l5_2 = nn.Conv3d(self.base_n_filter * 16, self.base_n_filter * 16, kernel_size=3, stride=1, padding=1,
                                   bias=True)
        ...
        self.conv3d_l4 = nn.Conv3d(self.base_n_filter * 2, self.n_classes, kernel_size=1, stride=1, padding=0,
                                   bias=True)
        self.conv3d_l4_dist = nn.Conv3d(self.base_n_filter * 2, self.n_classes, kernel_size=1, stride=1, padding=0,
                                   bias=True)
        self.conv3d_l4_ssl = nn.Conv3d(self.base_n_filter, 2, kernel_size=1, stride=1, padding=0,
                                       bias=True)
    ...
    def forward(self, x, phase, network_switch):
        ...
        out_d1 = self.conv_norm_lrelu_l4(out_d1)  # 64
        out_seg = self.conv3d_l4(out_d1)  # 1
        out_dist = self.conv3d_l4_dist(out_d1)
        ...
        return F.sigmoid(out_seg), F.sigmoid(out_rec),F.sigmoid(out_dist)


class MASSL_norm_feature_bank(nn.Module):
    def __init__(self, in_channels, n_classes, base_n_filter=8):
        super(MASSL_norm_feature_bank, self).__init__()
        # Define basic parameters
        self.in_channels = in_channels
        self.n_classes = n_classes
        self.base_n_filter = base_n_filter
        ...
        ###############################################################
        # Feature Bank Level 1 pathway
        self.fb_conv3d_l1_1 = nn.Conv3d(self.in_channels, self.base_n_filter, kernel_size=3, stride=1, padding=1,
                                     bias=True)
        self.fb_conv3d_l1_2 = nn.Conv3d(self.base_n_filter, self.base_n_filter, kernel_size=3, stride=1, padding=1,
                                     bias=True)

        # Feature Bank Level 2 pathway
        self.fb_conv3d_l2_1 = nn.Conv3d(self.base_n_filter, self.base_n_filter * 2, kernel_size=3, stride=2, padding=1,
                                   bias=True)
        self.fb_conv3d_l2_2 = nn.Conv3d(self.base_n_filter * 2, self.base_n_filter * 2, kernel_size=3, stride=1, padding=1,
                                   bias=True)

        # Feature Bank Level 3 pathway
        self.fb_conv3d_l3_1 = nn.Conv3d(self.base_n_filter * 2, self.base_n_filter * 4, kernel_size=3, stride=2, padding=1,
                                   bias=True)
        self.fb_conv3d_l3_2 = nn.Conv3d(self.base_n_filter * 4, self.base_n_filter * 4, kernel_size=3, stride=1, padding=1,
                                   bias=True)

        # Feature Bank Level 4 pathway
        self.fb_conv3d_l4_1 = nn.Conv3d(self.base_n_filter * 4, self.base_n_filter * 8, kernel_size=3, stride=2, padding=1,
                                   bias=True)
        self.fb_conv3d_l4_2 = nn.Conv3d(self.base_n_filter * 8, self.base_n_filter * 8, kernel_size=3, stride=1, padding=1,
                                   bias=True)

        # Feature Bank Level 5 pathway
        self.fb_conv3d_l5_1 = nn.Conv3d(self.base_n_filter * 8, self.base_n_filter * 16, kernel_size=3, stride=2, padding=1,
                                   bias=True)
        self.fb_conv3d_l5_2 = nn.Conv3d(self.base_n_filter * 16, self.base_n_filter * 16, kernel_size=3, stride=1, padding=1,
                                   bias=True)
    ...
    def forward(self, x, phase, network_switch):
        ...
        # Set trainable parameters given Labeled images:
        if phase == 'trainLabeled':
            feature_bank = network_switch['trainL_feature_bank']
            encoder = network_switch['trainL_encoder']
            decoder_seg = network_switch['trainL_decoder_seg']
            decoder_rec = network_switch['trainL_decoder_rec']

            # Set trainable parameters for Feature Bank
            for param in self.fb_conv3d_l1_1.parameters():
                param.requires_grad = feature_bank
            for param in self.fb_conv3d_l1_2.parameters():
                param.requires_grad = feature_bank
            for param in self.fb_conv3d_l2_1.parameters():
                param.requires_grad = feature_bank
            for param in self.fb_conv3d_l2_2.parameters():
                param.requires_grad = feature_bank
            for param in self.fb_conv3d_l3_1.parameters():
                param.requires_grad = feature_bank
            for param in self.fb_conv3d_l3_2.parameters():
                param.requires_grad = feature_bank
            for param in self.fb_conv3d_l4_1.parameters():
                param.requires_grad = feature_bank
            for param in self.fb_conv3d_l4_2.parameters():
                param.requires_grad = feature_bank
            for param in self.fb_conv3d_l5_1.parameters():
                param.requires_grad = feature_bank
            for param in self.fb_conv3d_l5_2.parameters():
                param.requires_grad = feature_bank

            # Set trainable parameters for UNet Encoder
            for param in self.e_conv3d_l1_1.parameters():
                param.requires_grad = encoder
            for param in self.e_conv3d_l1_2.parameters():
                param.requires_grad = encoder
        ...
        # Set trainable parameters given Unlabeled images:
        if phase == 'trainUnlabeled':
            feature_bank = network_switch['trainU_feature_bank']
            encoder = network_switch['trainU_encoder']
            decoder_seg = network_switch['trainU_decoder_seg']
            decoder_rec = network_switch['trainU_decoder_rec']
        ...
        ################################################################
        # Feature Bank
        out = self.fb_conv3d_l1_1(x)  # 16
        out = nn.InstanceNorm3d(self.base_n_filter)(out)
        out = self.lrelu(out)
        out = self.fb_conv3d_l1_2(out)  # 16
        out = nn.InstanceNorm3d(self.base_n_filter)(out)
        fb_level1 = self.lrelu(out)
        out = (fb_level1)

        out = self.fb_conv3d_l2_1(out)  # 32
        out = nn.InstanceNorm3d(self.base_n_filter * 2)(out)
        out = self.lrelu(out)
        out = self.fb_conv3d_l2_2(out)  # 32
        out = nn.InstanceNorm3d(self.base_n_filter * 2)(out)
        fb_level2 = self.lrelu(out)
        out = (fb_level2)

        out = self.fb_conv3d_l3_1(out)  # 64
        out = nn.InstanceNorm3d(self.base_n_filter * 4)(out)
        out = self.lrelu(out)
        out = self.fb_conv3d_l3_2(out)  # 64
        out = nn.InstanceNorm3d(self.base_n_filter * 4)(out)
        fb_level3 = self.lrelu(out)
        out = (fb_level3)

        out = self.fb_conv3d_l4_1(out)  # 128
        out = nn.InstanceNorm3d(self.base_n_filter * 8)(out)
        out = self.lrelu(out)
        out = self.fb_conv3d_l4_2(out)  # 128
        out = nn.InstanceNorm3d(self.base_n_filter * 8)(out)
        fb_level4 = self.lrelu(out)
        out = (fb_level4)

        out = self.fb_conv3d_l5_1(out)  # 256
        out = nn.InstanceNorm3d(self.base_n_filter * 16)(out)
        out = self.lrelu(out)
        out = self.fb_conv3d_l5_2(out)  # 256
        out = nn.InstanceNorm3d(self.base_n_filter * 16)(out)
        out_feature_bank = self.lrelu(out)
        ...
        e_level1 = self.lrelu(out)
        out = e_level1
        # out = torch.cat([e_level1, fb_level1], dim=1)
        ...
        e_level2 = self.lrelu(out)
        out = e_level2
        # out = torch.cat([e_level2, fb_level2], dim=1)
        ...
        e_level3 = self.lrelu(out)
        out = e_level3
        # out = torch.cat([e_level3, fb_level3], dim=1)
        ...
        e_level4 = self.lrelu(out)
        out = e_level4
        # out = torch.cat([e_level4, fb_level4], dim=1)
        ...
        out_d1 = self.inorm3d_l0_seg(out_d1)
        out_d1 = self.lrelu(out_d1)
        out_d1 = torch.cat([out_d1, e_level4, fb_level4], dim=1)  # 512

        out_d1 = self.conv_norm_lrelu_l1(out_d1)  # 512
        out_d1 = self.conv3d_l1(out_d1)  # 256
        out_d1 = self.norm_lrelu_upscale_conv_norm_lrelu_l1_seg(out_d1)  # 128
        out_d1 = torch.cat([out_d1, e_level3, fb_level3], dim=1)  # 256

        out_d1 = self.conv_norm_lrelu_l2(out_d1)  # 256
        out_d1 = self.conv3d_l2(out_d1)  # 128
        out_d1 = self.norm_lrelu_upscale_conv_norm_lrelu_l2_seg(out_d1)  # 64
        out_d1 = torch.cat([out_d1, e_level2, fb_level2], dim=1)  # 128

        out_d1 = self.conv_norm_lrelu_l3(out_d1)  # 128
        out_d1 = self.conv3d_l3(out_d1)  # 64
        out_d1 = self.norm_lrelu_upscale_conv_norm_lrelu_l3_seg(out_d1)  # 32
        out_d1 = torch.cat([out_d1, e_level1, fb_level1], dim=1)  # 64
        ...
        ##########################################################################
        # Decoder 2 for reconstruction
        out_d2 = self.upscale_conv_norm_lrelu_l0_rec(out_feature_bank)  # 256
        out_d2 = self.conv3d_l0_rec(out_d2)  # 256
        out_d2 = self.norm_lrelu_upscale_conv_norm_lrelu_l1_rec(out_d2)  # 128
        out_d2 = self.conv3d_l1_ssl(out_d2)  # 128
        out_d2 = self.norm_lrelu_upscale_conv_norm_lrelu_l2_rec(out_d2)  # 64
...
class MASSL_norm_feature_bank_one_neuron(nn.Module):
    def __init__(self, in_channels, n_classes, base_n_filter=8):
        super(MASSL_norm_feature_bank_one_neuron, self).__init__()
        # Define basic parameters
        self.in_channels = in_channels
        self.n_classes = n_classes
        self.base_n_filter = base_n_filter
        ...
        self.one_neuron_1 = nn.Conv3d(self.base_n_filter * 16, 1, kernel_size=1, stride=1, padding=0,
                                   bias=True)
        self.one_neuron_2 = nn.Conv3d(1, self.base_n_filter * 16, kernel_size=1, stride=1, padding=0,
                                   bias=True)
    ...
    def forward(self, x, phase, network_switch):
        ...
        if phase == 'trainUnlabeled':
            ...
            for param in self.conv3d_l4_ssl.parameters():
                param.requires_grad = decoder_rec
            for param in self.one_neuron_1.parameters():
                param.requires_grad = decoder_rec
            for param in self.one_neuron_2.parameters():
                param.requires_grad = decoder_rec
        ...
        ##########################################################################
        # Decoder 2 for reconstruction
        out_d2 = self.one_neuron_1(out_feature_bank)
        out_d2 = self.one_neuron_2(out_d2)
        out_d2 = self.upscale_conv_norm_lrelu_l0_rec(out_d2)  # 256
        out_d2 = self.conv3d_l0_rec(out_d2)  # 256
        out_d2 = self.norm_lrelu_upscale_conv_norm_lrelu_l1_rec(out_d2)  # 128
        out_d2 = self.conv3d_l1_ssl(out_d2)  # 128
        out_d2 = self.norm_lrelu_upscale_conv_norm_lrelu_l2_rec(out_d2)  # 64
