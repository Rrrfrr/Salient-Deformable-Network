...
class UNetCore3D(nn.Module):
    ...
    def make_decoder_seg(self):
        init_channels = self.init_channels
        self.up4conva_seg = nn.Conv3d(init_channels * 2, init_channels * 2, (1, 1, 1))
        self.up4_seg = nn.Upsample(scale_factor=2)  # mode='bilinear'
        self.up4convb_seg = BasicBlock(init_channels * 2, init_channels * 2)

        self.up3conva_seg = nn.Conv3d(init_channels * 2, init_channels * 2, (1, 1, 1))
        ...
        self.up1conv_seg = nn.Conv3d(init_channels, self.out_channels, (3, 3, 3), padding=(1, 1, 1))
        self.seg = nn.Conv3d(16, 5, (1, 1, 1))
    def forward(self, x):
        c1 = self.conv1a(x)
        ...
        output_seg = self.up1conv_seg(u2s)
        seg = self.seg(output_seg)
