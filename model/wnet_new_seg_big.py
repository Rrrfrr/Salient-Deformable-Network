...
class UNet3D_new(nn.Module):
    ...
    def __init__(self, in_channels=2, out_channels=5, init_channels=32, p=0.2,use_dilate=False):
    ...
    def make_encoder(self):
        ...
        if self.use_dilate:
            self.conv4a = DilatedBlock(init_channels * 8)
        else:
        ...
        self.conv4e = nn.Conv3d(init_channels * 8, init_channels * 4, (1, 1, 1), stride=(1, 1, 1), padding=(1, 1, 1))
        self.conv4f = nn.Conv3d(init_channels * 4, init_channels * 2, (1, 1, 1), stride=(1, 1, 1), padding=(1, 1, 1))
    ...
    def make_decoder_seg(self):
        init_channels = self.init_channels
        self.up4conva_seg = nn.Conv3d(init_channels * 8, init_channels * 4, (1, 1, 1))
        self.up4_seg = nn.Upsample(scale_factor=2)  # mode='bilinear'
        self.up4convb_seg = BasicBlock(init_channels * 4, init_channels * 4)

        self.up3conva_seg = nn.Conv3d(init_channels * 4, init_channels * 2, (1, 1, 1))
        self.up3_seg = nn.Upsample(scale_factor=2)
        self.up3convb_seg = BasicBlock(init_channels * 2, init_channels * 2)

        self.up2conva_seg = nn.Conv3d(init_channels * 2, init_channels, (1, 1, 1))
        self.up2_seg = nn.Upsample(scale_factor=2)
        self.up2convb_seg = BasicBlock(init_channels, init_channels)

        self.up1conv_seg = nn.Conv3d(init_channels, self.out_channels, (1, 1, 1))
        self.seg = nn.Conv3d(self.out_channels, 5, (1, 1, 1))
    ...
    def forward(self, x):
        ...
        u4s = self.up4conva_seg(c4d)
        u4 = self.up4(u4)
        u4s = self.up4_seg(u4s)
        u4 = u4 + c3
        u4s = u4s + c3
        u4 = self.up4convb(u4)
        u4s = self.up4convb_seg(u4s)



        u3 = self.up3conva(u4)
        u3s = self.up3conva_seg(u4s)
        u3 = self.up3(u3)
        u3s = self.up3_seg(u3s)
        u3 = u3 + c2
        u3s = u3s + c2
        u3 = self.up3convb(u3)
        u3s = self.up3convb_seg(u3s)


        u2 = self.up2conva(u3)
        u2s = self.up2conva_seg(u3s)
        u2 = self.up2(u2)
        u2s = self.up2_seg(u2s)

        u2 = u2 + c1
        u2s = u2s + c1
        u2 = self.up2convb(u2)
        u2s = self.up2convb_seg(u2s)


        output = self.up1conv(u2)
        outputs = self.up1conv_seg(u2s)
        seg = self.seg(outputs)
...
class WNet3D(nn.Module):
    def __init__(self, init_channels=16, use_dialte=False):
        ...
        self.unet = UNet3D_new(init_channels=self.init_channels, out_channels=16,use_dilate=use_dialte)
        #self.affine_net = UNet_affine(init_channels=self.init_channels, out_channels=16,use_dilate=use_dialte)
    ...
    def forward_unet_down(self, x):
        ...
        c1 =self.unet.conv1b(c1)
        c1d = self.unet.ds1(c1)

        c2 = self.unet.conv2a(c1d)
        c2 = self.unet.conv2b(c2)
        c2d = self.unet.ds2(c2)

        c3 = self.unet.conv3a(c2d)
        c3 = self.unet.conv3b(c3)
        c3d = self.unet.ds3(c3)

        c4 = self.unet.conv4a(c3d)
        c4 = self.unet.conv4b(c4)
        c4 = self.unet.conv4c(c4)
        c4d = self.unet.conv4d(c4)

        c4d = self.unet.conv4e(c4d)
        c4d = self.unet.conv4f(c4d)
    ...
    def forward(self, fix, move):
        ...
        return wrap, flow_grid, flow, affine,affine_grid,seg_fix